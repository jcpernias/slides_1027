...
- El supuesto *ST.5'* requiere que no exista correlación serial en el
  término de error:
  #+begin_export latex
  \[
...
- ¿Qué consecuencias tiene el incumplimiento de este supuesto? ¿Cómo
  puede detectarse y cómo debe procederse en presencia de
  autocorrelación?
...
- Los parámetros del modelo de regresión se estiman por MCO. Se
  obtienen estimaciones consistentes si se cumplen los supuestos
  *ST.1'*, *ST.2'* y *ST.3'*.

- Los contrastes de hipótesis sobre los parámetros se llevan a cabo
  utilizando una estimación de $\var(\hat{\bm\beta})$ robusta a
...
- Por tanto, el valor esperado de $y_t$ condicional a sus valores
...
Pueden obtenerse contrastes de autocorrelación que son válidos aun un
