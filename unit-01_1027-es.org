...
- No se cumple el supuesto *RLM.4*: $E(u|x_1) \neq 0$.

** Sesgo: ejemplo (II)

- El incumplimiento de *RLM.4* se debe a la omisión de la variable
  $x_2$, de forma que:
  #+begin_export latex
  \[
    u = v + \beta_2 x_2.
...
- La esperanza condicional de $u$ puede reescribirse:
  #+begin_export latex
  \[
    E(u|x_1) = E(v|x_1) + \beta_2 E(x_2|x_1).
  \]
  #+end_export

** Sesgo: ejemplo (y III)

Para que no se produzca sesgo, se debería cumplir $E(u|x_1) = 0$. Esto
ocurre cuando:

- $E(v|x_1) = 0$: una vez se tiene en cuenta la variable $x_2$, $x_1$
  no está relacionada con el término de error.

- O $\beta_2 = 0$ ($x_2$ no es relevante), o $E(x_2|x_1) = 0$ ($x_2$ y
  $x_1$ no están correlacionadas).
...
- Se omiten variables explicativas *relevantes*.

- Las variables omitidas están correlacionadas con alguna de las
