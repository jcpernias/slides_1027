...
- En primer lugar, se presentan algunos modelos comúnmente utilizados.


** Modelos estáticos

- *Modelo estático*: un cambio en una variable explicativa sólo tiene
  efectos en el periodo en que se produce y no tiene influencia en los
...
- *Modelo de retardos distribuidos*: los cambios en las variables
  explicativas tienen efectos en más de un periodo.
...
- En ausencia de perturbaciones, $u_t = 0$, en equilibrio dinámico las
  ...
  #+begin_export latex
  \[
    y^* = \alpha + (\beta_0 + \beta_1 + \beta_2) z^*.
  \]
  #+end_export


** COMMENT Ajuste a un nuevo equilibrio
...
- En un *modelo autorregresivo* los regresores son retardos de la
...
- ¿Qué condiciones tienen que cumplirse para que MCO tenga buenas
  propiedades exactas?
...
exógenas*. El término de error $u_t$ debe estar no correlacionado con
...
- $\Exp(u_t | \bm{x_{t-1}}, \bm{x_{t-2}}, \dots, \bm{x_{1}}) =
  0$: ausencia de correlación con valores pasados de $\bm{x_t}$.
...
- *Insesgadez*: si se cumplen *ST.1*, *ST.2* y *ST.3*.

- *Eficiencia*: si, además, se cumplen *ST.4* y *ST.5*. En este caso,
  se pueden usar las fórmulas de MCO de los errores típicos de
  $\hat{\beta_j}$.

- *Normalidad*: si, además, se cumple *ST.6*. Las fórmulas
...
1, 2, \dots, T\}$ es *estacionario* y *débilmente dependiente* y sigue
el modelo:
#+begin_export latex
\[
  y_t = \beta_0 + \beta_1 x_{1t} + \beta_2 x_{2t} + \dots + \beta_k x_{kt} + u_t
...
- Este supuesto no excluye retroalimentación entre las variables ni la
  inclusión de retardos de la variable dependiente.
...
- *Consistencia*: si se cumplen *ST.1'* a *ST.3'*.

- *Normalidad asintótica*: si además se cumplen *ST.4'* y *ST.5'*.
...
- Por otro lado, el supuesto *ST.1'* requiere explícitamente
  estacionariedad y dependencia débil de la variable dependiente y de
...
#  LocalWords:  Newey West ST HAC heteroskedasticity and consistent
#  LocalWords:  autocorrelation ARDL AR
