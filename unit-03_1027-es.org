...
contrastes de hipótesis cuando se cumple la hipótesis nula.
...
- Un estimador insesgado es consistente si su varianza converge a 0
  conforme aumenta el tamaño muestral.

- Un estimador sesgado es consistente si tanto el sesgo como su
  varianza convergen a 0 cuando aumenta el tamaño muestral.

** Distribución asintótica

- La *distribución asintótica* de un estimador se obtiene como el
  límite cuando $n \to \infty$ de una secuencia de distribuciones.
...
- La *inconsistencia* de $\bhat_i$ es la diferencia entre el límite
  del estimador y el parámetro poblacional.

- En el caso del modelo de regresión simple:
  #+begin_export latex
  \[
    \plim \bhat_{1} - \beta_{1} = \frac{\cov(x_{1}, u)}{\var(x_{1})}.
  \]
  #+end_export
...
- Si se omite la variable $x_2$, la inconsistencia del estimador MCO
  de $\beta_1$ es:
  #+begin_export latex
  \[
...
- Las estimaciones de MCO de las varianzas de los estimadores son
...
Si se cumplen los supuestos de Gauss-Markov, *RLM.1* a *RLM.5*, la
distribución asintótica del estimador MCO es normal:
  #+begin_export latex
  \[
...
- En muestras grandes no es necesario el supuesto de normalidad del
  término de error para usar los contrastes $t$ y $F$.

- También existen otros contrastes que sólo son válidos
  asintóticamente.
...
** Contraste de exclusión de variables
...
** Contraste $\LM$ de exclusión de variables
