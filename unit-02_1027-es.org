...
*Regla de decisión*: se rechaza $H_0$ (en favor de $H_1$) a un nivel de
...
- La hipótesis nula se compone de una o más combinaciones lineales
  cuyo valor es conocido. Por ejemplo:
...
- *Restricciones de exclusión múltiple*: En la hipótesis nula las
  pendientes de $q$ regresores son nulas:
  #+begin_export latex
  \[
...
*Modelo no restringido*: los parámetros $\beta_1, \dots, \beta_q$ son iguales a $0$ y sólo es válido bajo la $H_0$:
#+begin_export latex
\[
...
#+begin_export latex
\[
  F = \frac{(\Rsqnr - \Rsqr) / q}{\Rsqnr / (n - k - 1)}
\]
#+end_export
