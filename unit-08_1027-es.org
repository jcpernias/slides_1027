...
- De acuerdo con la *frecuencia* con que se observa a una variable,
  hablamos de series temporales anuales, trimestrales, mensuales, etc.
...
#+MATS: fig file:fig-08a_1027-infl.pdf
...
Si las tasas de crecimiento son moderadas, no hay grandes diferencias
entre las diferentes formas de calcularlas.
...
#+MATS: fig file:fig-08a_1027-gdp.pdf
...
#+MATS: fig file:fig-08a_1027-retail.pdf
...
- El coeficiente de autocorrelación de orden $h$ se define como
  #+begin_export latex
  \[
    \rho_h = \corr(y_t, y_{t-h}) =
...
Un *correlograma* es la representación gráfica de los coeficientes de
...
#+MATS: fig file:fig-08b_1027-ar1-ex-corr-*.pdf
...
- La *incorrelación asintótica* es una condición necesaria de la
  dependencia débil. Los coeficientes de autocorrelación convergen a 0
  conforme consideramos periodos más alejados: $\rho_h \to 0$ cuando
  $h \to \infty$.

- La dependencia débil exige, además, que esa convergencia sea lo
...
- $\cov(e_{t}, e_{t-h}) = 0$, para todo $h > 0$.
...
#+MATS: fig file:fig-08b_1027-wn-ts-*.pdf


** Características

- Un ruido blanco es un proceso estacionario y débilmente dependiente.
...
- Un ruido blanco no tiene *memoria*: el valor que toma en un periodo no
  tiene ninguna influencia sobre los valores futuros.


** Autocorrelación

#+MATS: figcol file:fig-08b_1027-wn-lag-*.pdf 0.5
...
** COMMENT Esperanza de un proceso AR(1)
...
** COMMENT Varianza de un proceso AR(1)
...
** COMMENT Autocovarianzas de un proceso AR(1) (I)
...
** COMMENT Autocovarianzas de un proceso AR(1) (II)
...
** COMMENT Autocovarianzas de un proceso AR(1) (y III)
...
** COMMENT Autocorrelaciones de un proceso AR(1)
...
Un proceso AR(1) estable cumple las condiciones de estacionariedad
débil:
#+begin_export latex
\begin{gather*}
  E(y_t) = 0, \\
  \var(y_t) = \sigma^2_y = \sigma^2_e / (1 - \rho^2), \\
  \cov(y_{t}, y_{t-h}) = \rho^h \sigma^2_y \quad \text{para $h > 0$}.
\end{gather*}
#+end_export

** Dependencia débil

Un proceso AR(1) estable es débilmente dependiente. Los coeficientes
de autocorrelación:
#+begin_export latex
\[
  \corr(y_{t}, y_{t-h}) = \rho^h,
\]
#+end_export
convergen a 0 conforme $h \to \infty$ y la convergencia es geométrica.
...
#+MATS: fig file:fig-08b_1027-ar8-ts-*.pdf

Proceso AR(1), $\rho = 0.2$, $\sigma^2_e = 4$, $T = 60$.
#+MATS: fig file:fig-08b_1027-ar2-ts-*.pdf
...
#+MATS: col 0.5
Proceso AR(1), $\rho = 0.8$, $\sigma^2_e = 4$, $T = 60$.
#+MATS: fig file:fig-08b_1027-ar8-lag-*.pdf

#+MATS: col 0.5
Proceso AR(1), $\rho = 0.2$, $\sigma^2_e = 4$, $T = 60$.
#+MATS: fig file:fig-08b_1027-ar2-lag-*.pdf
...
Proceso AR(1), $\rho = 0.8$, $\sigma^2_e = 4$, $T = 60$.
#+ATTR_LATEX: :scale 0.8
#+MATS: fig file:fig-08b_1027-ar1-p8-corr-*.pdf

#+MATS: col 0.5
Proceso AR(1), $\rho = 0.2$, $\sigma^2_e = 4$, $T = 60$.
#+ATTR_LATEX: :scale 0.8
#+MATS: fig file:fig-08b_1027-ar1-p2-corr-*.pdf
...
** COMMENT Autocovarianzas y autocorrelaciones
...
#+MATS: fig file:fig-08b_1027-rw-ts-*.pdf
...
#+MATS: col 0.5
#+ATTR_LATEX: :scale 0.8
#+MATS: fig file:fig-08b_1027-ar1-rw-corr-*.pdf

** Comparación con AR(1)

- Sustituyendo recursivamente, un proceso AR(1) estable puede
  expresarse como:
  #+begin_export latex
  \[
    y_t =  e_t +  \rho e_{t-1} + \rho^2 e_{t-2} + \rho^3 e_{t-3}  + \dots
  \]
  #+end_export
  La influencia de innovaciones en periodos anteriores decrece con el
  paso del tiempo.

- Un paseo aleatorio puede expresarse como:
  #+begin_export latex
  \[
    y_t =  e_t +  e_{t-1} + e_{t-2} + e_{t-3}  + \dots
  \]
  #+end_export
  Las innovaciones no pierden influencia con el paso del tiempo.
...
#+MATS: fig file:fig-08b_1027-rwd-ts-*.pdf
...
#+MATS: fig file:fig-08b_1027-tst-ts-*.pdf
...
#+MATS: fig file:fig-08b_1027-lp-ts-*.pdf
...
\]
#+end_export
#+MATS: fig file:fig-08b_1027-infl-ts-*.pdf
...
\]
#+end_export
#+MATS: fig file:fig-08b_1027-dinfl-ts-*.pdf
...
#  LocalWords:  COMMENT AR Autocovarianzas
